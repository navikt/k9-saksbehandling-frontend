...
const AktivitetRad: React.FC<ownProps> = ({ overstyring, index, handleRediger, visOverstyringSkjema, handleSlett, loading, setLoading }) => {
    ...
    return (
        <Table.ExpandableRow
            ...
            open={ekspandert}
        >
            <Table.DataCell onClick={() => setEkspandert(!ekspandert)}>
                {dayjs(fom).format('DD.MM.YYYY')}
            </Table.DataCell>
            <Table.DataCell onClick={() => setEkspandert(!ekspandert)}>
                {dayjs(tom).format('DD.MM.YYYY')}
            </Table.DataCell>
            ...
            <Table.DataCell>
                ...
                <Button size="xsmall" variant="tertiary" icon={<Delete aria-hidden />} onClick={() => handleSlett(id)} disabled={visOverstyringSkjema} loading={loading}>
                    Slett
                </Button>
            </Table.DataCell>
        </Table.ExpandableRow>
