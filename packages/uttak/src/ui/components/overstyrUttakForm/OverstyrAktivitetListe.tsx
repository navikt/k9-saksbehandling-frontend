...
const OverstyrAktivitetListe: React.FC<ownProps> = ({ fields, loading }) => {
    ...
    const utledAtkivitetNavn = (
        ...
        if (arbeidsforhold[OverstyrUttakFormFieldName.AKTØR_ID])
            return `${arbeidsforhold[OverstyrUttakFormFieldName.TYPE]} ${arbeidsforhold[OverstyrUttakFormFieldName.AKTØR_ID]}`;
        if (arbeidsforhold[OverstyrUttakFormFieldName.ORGANISASJONSNUMMER])
            return `${arbeidsforhold[OverstyrUttakFormFieldName.TYPE]} ${arbeidsforhold[OverstyrUttakFormFieldName.ORGANISASJONSNUMMER]}`;
        if (arbeidsforhold[OverstyrUttakFormFieldName.ARBEIDSFORHOLD_ID])
            return `${arbeidsforhold[OverstyrUttakFormFieldName.TYPE]} ${arbeidsforhold[OverstyrUttakFormFieldName.ARBEIDSFORHOLD_ID]}`;
