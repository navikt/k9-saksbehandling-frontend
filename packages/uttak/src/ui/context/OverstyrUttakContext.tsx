...
import {
    Arbeidsforhold,
    OverstyrbareAktiviteterResponse,
    OverstyringUttak,
    OverstyrtResponse,
} from '../../types';
...
type OverstyrUttakContextType = {
    ...
    arbeidsgivere: null | OverstyrtResponse['arbeidsgiverOversikt']['arbeidsgivere'];
...
export const OverstyrUttakContextProvider = ({ children }) => {
    ...
    const [arbeidsgivere, setArbeidsgivere] = React.useState<
        OverstyrtResponse['arbeidsgiverOversikt']['arbeidsgivere'] | null
    >(null);

    const hentOverstyrte = async () => {
        setLasterOverstyringer(true);
        const apiResult: OverstyrtResponse = await get(endpoints.behandlingUttakOverstyrt, httpErrorHandler).then(
            (response: OverstyrtResponse) => response
        );
